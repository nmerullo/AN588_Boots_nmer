---
title: "nmer_FinalHomeworkCode_05"
...
Female body mass significantly predicts home range. For each 1 unit increase of female body mass, range increases by 1.0868. 

The model explains 53% of the variation and the F-statistic is significant.
...
summary(boot_r.bm)
```
**Final code commentary:**
I wanted to try boostrapping again because my standard errors and standard deviations were not matching and thus my distribution of samples was not meeting the central limit theorem. I was thinking that this had to do with resample not letting me specify a number of samples pulled from the data frame (an error throws if I add an argument). So I rewrote the bootsrap using similar ideas but on my own. What I have below I REALLY feel like should work, but doesn't. It doesn't throw an error but it basically just spits out the coefficients of the linear regression from the original sample, as if I didn't pull 100 samples from those two columns then replicated that 1000 times THEN fit a model on top of it. I have spent too long looking at this so I will let it be. At least I have something that functions.

```{r take2}
tryingagain <- lm(
  log(d$HomeRange_km2)~log(d$Body_mass_female_mean), 
  data = as.data.frame(replicate(1000, sample(d[, c("HomeRange_km2", "Body_mass_female_mean")], n = 100, replace = TRUE)))
  )
tryingagain
```
...
**Final commentary** I must have been looking at the wrong confidence intervals when I wrote the above, or maybe the randomness of the bootsrap produced some off CIs but the sampling distribution has a nice narrow confidence distribution, especially compared to the intercept (B0).